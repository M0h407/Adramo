...
public class Metodes {
	...
	public static Double calcularNomina(String dni, int id) {
		...
		if (diaNomin != 1) {
			...
			String consulta = "SELECT distancia, data_envio FROM envios WHERE id_treballador = " + id;
			Statement stmt;
			ResultSet rs;
			try {
	...
	public static String generarCarpetaNomina(String Dni) {
		...
		String dirFinal = "\\\\192.168.1.69\\AdramoNomines\\Nòmines\\" + Dni + "\\" + strAny + "\\" + mesFin + "\\";
		String drFinn = null;
		...
		if (!(dirFin.exists())) {
			...
			drFinn = dirFinal + Dni + "-" + mesFin + "-" + strAny + "-" + 1 + ".pdf";
		} else {
			File[] llista = dirFin.listFiles();
			int numArch = llista.length + 1;
			drFinn = dirFinal + Dni + "-" + mesFin + "-" + strAny + "-" + numArch + ".pdf";
		}
		return drFinn;
	}
